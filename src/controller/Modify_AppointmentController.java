...
//Start Date Conversions
       ...
       //
       String lastUpdated = java.time.LocalDateTime.now().format(formatter);
       String formattedLastUpdated = timeConvert.convertToUTCString(lastUpdated);
       System.out.println(formattedLastUpdated);
       
       //
       if(withinBusinessHours(startESTDateTimeLocalDateTime,endESTDateTimeLocalDateTime) && validateAppointment() && appointmentOverlapping(Integer.parseInt(appointmentIdTxtBox.getText()),startESTDateTimeLocalDateTime,endESTDateTimeLocalDateTime)==false){      
       DBQuery dbQuery = new DBQuery();
       int appointmentID = Integer.parseInt(appointmentIdTxtBox.getText());
       ...
       dbQuery.updateAppointmentSQL(appointmentID, title, description, location, formattedLastUpdated, type, startUTCDateTimeString, endUTCDateTimeString, customerID, userID, contactID);
       ...
       //Stop
        ...
        modifyAppointmentLabel.setText(this.rb.getString("modifyAppointment"));
    ...
    /**
     *import appointment info from appointment form tableview
     * @param appointment appointment 
     */
    public void importAppointmentInfo(Appointment appointment){
      for (Contact checkContact : Lists.getAllContacts()) {
      if (checkContact.getContactName() == appointment.getContact()){
      Contact contact = checkContact;
      contactComboBox.setValue(contact);
      }
      }
    Appointment getSelectedAppointmentName = appointment; 
    appointmentIdTxtBox.setText(String.valueOf(appointment.getAppointmentID()));
    titleTxtBox.setText(appointment.getTitle());
    descriptionTxtBox.setText(appointment.getDescription());
    locationTxtBox.setText(appointment.getLocation());
    typeTxtBox.setText(appointment.getType());
    customerIdTxtBox.setText(Integer.toString(appointment.getCustomerID()));
    userIdTxtBox.setText(Integer.toString(appointment.getUserID()));
    //Dates here
    //Start
    String startDate = appointment.getStartDateTime().substring(0, 10);
    String startHour = appointment.getStartDateTime().substring(11, 13);
    String startMin = appointment.getStartDateTime().substring(14, 16);
    startDateTimeDatePicker.setValue(LocalDate.parse(startDate));
    startDateTimeHourComboBox.setValue(startHour);
    startDateTimeMinComboBox.setValue(startMin);
    //End
    String endDate = appointment.getEndDateTime().substring(0, 10);
    String endHour = appointment.getEndDateTime().substring(11, 13);
    String endMin = appointment.getEndDateTime().substring(14, 16);
    endDateTimeDatePicker.setValue(LocalDate.parse(endDate));
    endDateTimeHourComboBox.setValue(endHour);
    endDateTimeMinComboBox.setValue(endMin);
      }
     ...
     */
    public boolean appointmentOverlapping(int appointmentID, LocalDateTime startDate, LocalDateTime endDate){
         Exceptions exception = new Exceptions();
         TimeConvert timeConvert = new TimeConvert();
         DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss");
        ...
        for (Appointment appointment : Lists.getMonthlyAppointments()) {
            ...
            if((startDate.isBefore(compareLDTEnd)) && (endDate.isAfter(compareLDTStart)) && appointment.getAppointmentID() != appointmentID){
            // 
            exception.popUp(this.rb.getString("Overlapping_Appointment"),this.rb.getString ("Overlapping_appointment_with_Start")+ appointment.getStartDateTime() +this.rb.getString(" and_End ") + appointment.getEndDateTime());
            return true;
            }
        }
